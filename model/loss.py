...
def bce_loss(output, target, reduction="mean", return_none_reduction=False):
    """
    ...
    """
    if return_none_reduction:
        loss_each = F.binary_cross_entropy_with_logits(output.squeeze(), target.float(), reduction='none')
...
def weighted_bce_loss(output, target, weight=torch.tensor([1/31,1.0], dtype=torch.float), return_none_reduction=False):
    #target = 1 is positive example, target = 0 is negative example
    #Treat target as a mask to identify the positive examples (1)
    #Use mask to create final weights
    weight_tensor = target.clone().float()
    weight_tensor[target == 0] = 1/31
    weight_tensor[target == 1] = 1.0

    if return_none_reduction:
        loss_each = F.binary_cross_entropy_with_logits(output.squeeze(), target.float(), weight=weight_tensor, reduction="none")
        loss = torch.mean(loss_each)
        return loss, loss_each
    else:
        loss = F.binary_cross_entropy_with_logits(output.squeeze(), target.float(), weight=weight_tensor)
...
def distance_loss(inp, target, **kwargs):
    # print('in distance loss')
    # print(inp[:10])
    # print(target[:10])
    return F.cross_entropy(inp.neg(), target)

def entailment_cones_loss(inp, target, **kwargs):
