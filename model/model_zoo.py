...
class RGCN(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_layers, activation, in_dropout=0.1, hidden_dropout=0.1, output_dropout=0.0):
        ...
        self.num_rels = 3
        # input layer
        self.layers.append(RelGraphConv(in_dim, hidden_dim, self.num_rels, regularizer='basis', 
                                        activation=activation, dropout=in_dropout))
        # hidden layers
        for l in range(num_layers - 1):
            self.layers.append(RelGraphConv(hidden_dim, hidden_dim, self.num_rels, regularizer='basis', 
                                        activation=activation, dropout=hidden_dropout))
        # output layer
        self.layers.append(RelGraphConv(hidden_dim, out_dim, self.num_rels, regularizer='basis', 
                                        activation=None, dropout=output_dropout))
...
class attnReadout(nn.Module):
    def __init__(self, input_dim=None, hidden_dim=None):
        ...
        self.attn_w = nn.Linear(input_dim, hidden_dim)

    def forward(self, q, g, pos=None, dist=None):
        extended_q = dgl.broadcast_nodes(g, q)
        h = g.ndata['h']
        attn_h_q = torch.sum(h * self.attn_w(extended_q), dim=-1)
        g.ndata['unorm_a'] = attn_h_q
        g.ndata['a'] = dgl.softmax_nodes(g, 'unorm_a').unsqueeze(-1)
...
class MeanReadout(nn.Module):
    ...
    def forward(self, g, pos=None, dist=None):
        return dgl.mean_nodes(g, 'h')

class MeanReadout1Hop(nn.Module):
    def __init__(self, max_dist=3):
        super(MeanReadout1Hop, self).__init__()
        self.weights = nn.Embedding(max_dist, 1)
        self.nonlinear = F.softplus
        w_list = [1, 1] + [0] * (max_dist - 2)
        init_weight = torch.unsqueeze(torch.cuda.FloatTensor(w_list), 1)
        self.weights.weight = torch.nn.Parameter(init_weight)
        self.weights.weight.requires_grad = False
    
    def forward(self, g, pos=None, dist=None):
        g.ndata['a'] = self.nonlinear(self.weights(dist))
        return dgl.mean_nodes(g, 'h', 'a')
        
class WeightedMeanReadout(nn.Module):
    def __init__(self, position_vocab_size=3):
        super(WeightedMeanReadout, self).__init__()
        self.weights = nn.Embedding(position_vocab_size, 1)
        self.nonlinear = F.softplus
    
    def forward(self, g, pos, dist=None):
        g.ndata['a'] = self.nonlinear(self.weights(pos))
        return dgl.mean_nodes(g, 'h', 'a')
...
class FLP(nn.Module):
    def __init__(self, l_dim, r_dim, hidden_dim, k, non_linear=F.tanh):
        ...
        self.weight = nn.Linear(l_dim, r_dim)
        self.ffn = nn.Sequential(
            nn.Linear(l_dim*2+r_dim*3, hidden_dim),
            activation,
            nn.Linear(hidden_dim, k, bias=False)
        )
        self.u_R = nn.Linear(k, 1, bias=False)
    ...
    def forward(self, e1, e2, q):
        ...
        return: tensor of size (*, 1)
        """
        c1 = torch.mul(q, self.weight(e1))
        c2 = torch.mul(q, self.weight(e2))
        c1 = self.dropout(c1)
        c2 = self.dropout(c2)
        return self.u_R(self.f(self.ffn(torch.cat((e1, e2, q, c1, c2), 1))))

class RawMLP(nn.Module):
    def __init__(self, l_dim, r_dim, hidden_dim, k, non_linear=F.tanh, dropout=0.1):
    ...
    def forward(self, e, q, batch_norm_flag=True):
        """
        e1: tensor of size (*, l_dim)
        e2: tensor of size (*, r_dim)
        ...
        tmp = torch.cat((e, q), 1)
        if batch_norm_flag:
            return self.dropout(self.u_R(self.batch_norm(self.f(self.ffn(tmp)))))
        else:
            return self.dropout(self.u_R(self.f(self.ffn(tmp))))

class RawMLP_combined(RawMLP):
    def forward(self, e, batch_norm_flag=True):
        """
        e: tensor of size (*, l_dim * 2)

        return: tensor of size (*, 1)
        """
        if batch_norm_flag:
            return self.dropout(self.u_R(self.batch_norm(self.f(self.ffn(e)))))
        else:
            return self.dropout(self.u_R(self.f(self.ffn(e))))
...
class RawNTN(nn.Module):
    def __init__(self, l_dim, r_dim, k=5, non_linear=torch.tanh, dropout=False):
    ...
    def forward(self, e, q):
        ...
        return: tensor of size (*, 1)
        """
        return self.u_R(self.dropout(self.f(self.W(e, q) + self.V(torch.cat((e, q), 1)))))
...
class TMN(nn.Module):
    ...
    def forward(self, e1, e2, q, force_return_all_scores=False):
        ec1 = e1 * self.control_l(torch.cat((e1, q), -1))
        ec2 = e2 * self.control_r(torch.cat((e2, q), -1))
        e = torch.cat((e1, e2), 1)
        ...
        if force_return_all_scores or self.training:
