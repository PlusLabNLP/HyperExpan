...
class MobiusLinearMM(nn.Module):
    def __init__(self, in_dim, out_dim, bias_term=True, nonlin=None, **kwargs):
        self.bias_term = bias_term
        self.nonlin = nonlin

        self.ball = gt.PoincareBall()
        k_w = (6 / (in_dim + out_dim)) ** 0.5 # xavier uniform
        self.w = gt.ManifoldParameter(gt.ManifoldTensor(dim, dim).uniform_(-k_w, k_w))
        if bias_term:
            bias = torch.empty(out_dim)
            self.b = gt.ManifoldParameter(pmath.expmap0(bias.normal_() * 1e-3, k=self.ball.k), manifold=self.ball)

    def forward(self, input):
        e = pmath.mobius_matvec(self.w, input, k=self.ball.k)
        if self.bias_term:
            e = pmath.mobius_add(e, self.b, k=self.ball.k)
        if self.nonlin is not None:
            e = pmath.mobius_fn_apply(nonlin, e, k=k)
        e = pmath.project(e, k=k)
        return e
    ...
    def __init__(self, *args, hyperbolic_input=True, hyperbolic_bias=True, nonlin=None, c=1.0, **kwargs):
        ...
        if self.bias is not None:
            if hyperbolic_bias:
                ...
                with torch.no_grad():
                    self.bias.set_(pmath.expmap0(self.bias.normal_() * 1e-8, k=self.ball.k))
        with torch.no_grad():
...
class MobiusConcat(nn.Module):
    def __init__(self, output_dim, input_dims, second_input_dim=None, third_input_dim=None, nonlin=None,
        ...
        if manifold == 'PoincareBall':
            self.ball = gt.PoincareBall(c=curvature)
        elif manifold == 'Hyperboloid':
            self.ball = gt.Lorentz(k=curvature)
